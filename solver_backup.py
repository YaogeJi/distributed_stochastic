...
class SMD_constrained(OnlineSolver):
    # Seems like dual averaging updates. Not in the theoretical paper.
    def __init__(self, generator, network, gamma, lmda, args, prox=0) -> None:
...
class MirrorDescent_ATC_V1(DualAveraging):
    def fit(self):  
        ...
        for i in range(int(len(self.generator)/self.computation)):
            ...
            c_loss = np.linalg.norm(iter_theta - repeat_avg_iter_theta, ord=1) ** 2 / (self.generator.m)
            wandb.log({"iter_loss (log scale)": np.log(loss),"l1_loss (log scale)":np.log(l1_loss), "consensus_loss (log scale):": np.log(c_loss)}, step=i)
        return True
...
class MirrorDescent_ATC_V2(DualAveraging):
    # This version is not in the theoretical analysis. The shrinkage is before the dual communicating.
    def fit(self):  
        loss_matrix = []
        theta_matrix = []
        mu_matrix = []
